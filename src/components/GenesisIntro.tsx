...
export function GenesisIntro({ onDone }: { onDone: () => void }) {
  ...
  useEffect(() => {
    ...
    type P = { x: number; y: number; tx: number; ty: number; vx: number; vy: number; c: string; r: number; born: number };
    ...
    for (let i = 0; i < N; i++) {
      ...
      particles.push({
        ...
        c: Math.random() < 0.4 ? letterColor(t.x) : colors[Math.floor(Math.random() * colors.length)],
